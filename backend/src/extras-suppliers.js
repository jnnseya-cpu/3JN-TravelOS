...
export function supplierDoors() {
  return [
    ...
    { channel: 'flights-lcc', provider: 'Kiwi Tequila (invitation-only)', envVar: 'TEQUILA_API_KEY', signup: 'https://tequila.kiwi.com — B2B partnerships (now invitation-only)', covers: 'Ryanair/Jet2/Wizz bookable content', fallback: 'ops desk books on airline site' },
    { channel: 'flights-travelfusion', provider: 'Travelfusion (LCC consolidator)', envVar: 'TRAVELFUSION_LOGIN_ID + TRAVELFUSION_PASSWORD (optional TRAVELFUSION_MODE=live)', signup: 'https://www.travelfusion.com — Flight API, sales-led (also the accredited route into Ryanair’s approved-OTA programme)', covers: 'Ryanair/easyJet/Jet2/Wizz/Vueling at consolidator fares — undercuts Duffel on European short-haul; adapter BUILT + wired (fail-closed until certified)', fallback: 'Duffel fare shown; ops desk books on airline site' },
    ...
    open: d.channel === 'flights' ? !!env.DUFFEL_TOKEN
      ...
      : d.channel === 'flights-travelfusion' ? !!(env.TRAVELFUSION_LOGIN_ID && env.TRAVELFUSION_PASSWORD)
