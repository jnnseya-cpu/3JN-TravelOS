...
export function operatorConfirm(bookingId) {
  ...
  if (pa.kind === 'change') {
    ...
    if (wasLiveTicketed) {
      ...
      // Supersede any lingering open reissue ticket for THIS booking so the desk
      // never sees duplicate/stale reissue work-orders (the "2 open for the same
      // booking" pile-up). Only one reissue is ever in flight per booking.
      try { for (const t of listSupportTickets('open')) if (t.intent === 'ops-reissue' && t.bookingId === b.id) resolveSupportTicket(t.id, { note: 'Superseded by a newer change request on this booking.', agent: 'system' }); } catch { /* best-effort */ }
