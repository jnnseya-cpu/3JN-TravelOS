...
export function socialPost({ destination, platform, tone: t, variant = 0 } = {}, ctx = {}) {
  ...
  const bodies = [
    `One sentence in, 3JN Travel OS builds the whole thing — flights, hotel, visa check, even eSIM — into one booking, and you can pay monthly.`,
...
export function travelAdvert({ destination, tone: t, variant = 0 } = {}, ctx = {}) {
  ...
  const primary = `${tn.open}: describe your ${dest} trip in one sentence and 3JN Travel OS builds it — flights, hotel, transfers, visa & eSIM — into one booking, with monthly instalments and a 24/7 price guard.`;
...
export function emailCampaign({ destination, tone: t, variant = 0 } = {}, ctx = {}) {
  ...
  const body = [
    ...
    ``,
    `With 3JN Travel OS you describe the trip in one sentence and the AI builds the whole thing — flights, hotel, transfers, a visa-approval check and eSIM — into one protected booking. Prefer to spread the cost? Pay a deposit and the rest in monthly instalments.`,
    ``,
...
<p class="sub">Describe your trip in one sentence. Get flights, hotel, transfers, a visa-approval check and eSIM in one booking — with monthly instalments.</p>
