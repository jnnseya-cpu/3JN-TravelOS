...
export function quoteChange(booking, changes, { todayISO } = {}) {
  ...
  if (changes.kind === 'date') {
    ...
    // Airline fare difference: for a live fare it is confirmed with the carrier
    // at re-issue (shown as its own line, £0 here until the supplier returns it).
    lines.push({ label: isLive ? 'Airline fare difference (confirmed at re-issue)' : 'Fare difference', amountGbp: 0, deferred: isLive });
...
export function applyChange(booking, quote) {
  ...
  const flight = (booking.option?.components || []).find((c) => c.type === 'flight');
  if (changes.kind === 'date' && flight?.details) {
    if (flight.details.outbound) flight.details.outbound.date = changes.newDate;
    // Keep the same trip length by shifting the return proportionally if present.
