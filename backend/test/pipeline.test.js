...
test('referral records attribution but grants NO points at signup', () => {
  // Loyalty points are earned only on a completed booking — never a signup bonus
  // or a referral signup. The referral is rewarded (250 ACU + revshare) on the
  // friend's first PAID booking instead (see the paid-referral test below).
  const referrer = createUser({ name: 'Referrer' });
  const before = referrer.referrals;
  const friend = createUser({ name: 'Friend', referredByCode: referrer.referralCode });
  assert.equal(friend.points, 0, 'no signup/referral points — starts at zero');
  assert.equal(getUserById(referrer.id).referrals, before + 1, 'referral attribution is still recorded');
});
...
test('LIVE_MODE: no free AI for guests; demo accounts fail closed', async () => {
  ...
  try {
    ...
    const post = (p, body, h = {}) => fetch(base + p, { method: 'POST', headers: { 'content-type': 'application/json', ...h }, body: JSON.stringify(body) });
    try {
      // NO FREE AI: a guest gets zero free AI — the very first search returns a
      // sign-in gate (new accounts get starter ACUs to fund searches).
      const ip = { 'x-forwarded-for': `203.0.113.${(Date.now() % 200) + 1}` };
      const r = await (await post('/api/plan', { text: `2 adults to Rome for 4 nights Oct ${Date.now()}, flights hotel` }, ip)).json();
      assert.equal(r.stage, 'signup-required', 'a guest is sign-in gated on the FIRST AI search');
      assert.equal(r.reason, 'ai-requires-account');
